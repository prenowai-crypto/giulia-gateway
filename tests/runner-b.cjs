  ...
  for (const userText of userTurns) {
    ...
    if (!firstUserTurnHandled) {
      ...
      if (detectedLang !== 'it') {
        ...
        console.log(`  ⚖️  Injecting ${detectedLang.toUpperCase()} disclosure via session.update (Soluzione B fixed)`);

        // Soluzione B fixed — stesso pattern del gateway v7.4.37:
        // 1. session.update con instructions SEMPLICI ("say exactly X") — no "don't call tools"
        // 2. Aspetto session.updated (evita race condition)
        // 3. response.create → disclosure
        // 4. Aspetto response.done
        // 5. session.update con prompt originale
        // 6. Aspetto session.updated
        // 7. response.create → vera risposta

        // Step 1-3: aggiorno session, aspetto conferma, chiedo response
        await new Promise((resolve, reject) => {
          const to = setTimeout(() => {
            ws.off('message', handler);
            reject(new Error(`disclosure step 1-3 timeout`));
          }, TURN_TIMEOUT_MS);
          let sessionAcked = false;
          let discText = '';
          const handler = (raw) => {
            let m; try { m = JSON.parse(raw); } catch { return; }
            if (m.type === 'session.updated' && !sessionAcked) {
              sessionAcked = true;
              send({ type: 'response.create' });
              return;
            }
            if (m.type === 'response.output_text.delta' && m.delta) discText += m.delta;
            if (m.type === 'response.output_text.done' && m.text) discText = m.text;
            if (m.type === 'response.text.delta' && m.delta) discText += m.delta;
            if (m.type === 'response.text.done' && m.text) discText = m.text;
            if (m.type === 'response.output_item.done' && m.item?.content) {
              for (const c of m.item.content) {
                if ((c.type === 'output_text' || c.type === 'text') && c.text) discText = c.text;
              }
            }
            if (m.type === 'response.done') {
              if (discText.trim()) {
                aiReplies.push(discText.trim());
                events.push({ type: 'ai', text: discText.trim() });
              }
              clearTimeout(to);
              ws.off('message', handler);
              resolve();
            ...
            if (m.type === 'error') {
              clearTimeout(to);
              ws.off('message', handler);
              reject(new Error(`disclosure error: ${JSON.stringify(m.error || m)}`));
            }
          };
          ws.on('message', handler);
          send({
        ...
        // Step 5-6: ripristino prompt originale, aspetto session.updated
        console.log(`  ▶️  Disclosure delivered → restoring original session`);
        await new Promise((resolve, reject) => {
          const to = setTimeout(() => {
            ws.off('message', handler);
    ...
    // Step 7 (or normal path per italiano): response.create per la vera risposta
    send({ type: 'response.create' });
