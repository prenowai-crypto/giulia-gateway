...
async function detectLanguageWithLLM(text) {
  try {
    ...
    // Fallback minimo regex
    const t = (text || '').toLowerCase();
    if (/¿|¡/.test(t)) return 'es';
    if (/\b(hola|hablas|español|quisiera|reservar|gracias)\b/i.test(t)) return 'es';
    if (/\b(bonjour|bonsoir|je voudrais|réserver|merci|français)\b/i.test(t)) return 'fr';
    if (/\b(guten tag|guten abend|ich möchte|reservieren|deutsch)\b/i.test(t)) return 'de';
    if (/\b(hello|hi|good morning|good evening|i would like|i'd like|thanks)\b/i.test(t)) return 'en';
    return 'it';
  }
}
  ...
  for (const userText of userTurns) {
    ...
    if (!firstUserTurnHandled) {
      ...
      if (detectedLang !== 'it') {
        ...
        // First response: forced disclosure
        await new Promise((resolve, reject) => {
          const to = setTimeout(() => {
            ws.off('message', handler);
            reject(new Error(`disclosure response timeout`));
          }, TURN_TIMEOUT_MS);
          let discText = '';
          const handler = (raw) => {
