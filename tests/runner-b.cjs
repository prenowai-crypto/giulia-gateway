...
function assertLanguage(actual, spec) {
  ...
  const markers = {
    it: /\b(sono|prenotazione|grazie|salve|buongiorno|perfetto|un attimo|certo|va bene)\b/,
    en: /\b(hello|thank|reservation|booking|please|sure|one moment|great|confirmed|would you)\b/,
    fr: /\b(bonjour|réservation|merci|s'il vous plaît|parfait|un instant|bien sûr)\b/,
    de: /\b(hallo|reservierung|danke|bitte|einen moment|natürlich|perfekt)\b/,
    es: /\b(hola|reserva|gracias|por favor|un momento|perfecto|claro)\b/,
  };
  ...
  for (let b = 0; b < totalBatches; b++) {
    ...
    if (failed.length > 0) {
      ...
      for (const r of failed) {
        md += `### ${r.id} — \`${r.category}\`\n**Description**: ${r.description}\n\n**Reason**: ${r.failReason}\n\n**Tool calls made** (${r.toolCalls.length}):\n\`\`\`json\n${JSON.stringify(r.toolCalls, null, 2)}\n\`\`\`\n\n**Last AI reply**: ${(r.aiReplies[r.aiReplies.length - 1] || '(none)').substring(0, 300)}\n\n**Duration**: ${r.duration}ms\n\n---\n\n`;
      }
    }
