  ...
  for (const userText of userTurns) {
    ...
    if (!firstUserTurnHandled) {
      ...
      if (detectedLang !== 'it') {
        ...
        console.log(`  ⚖️  Forcing ${detectedLang.toUpperCase()} disclosure (unified response)`);
        // v8 — Unified response (allineato al gateway v7.4.38):
        // UN singolo response.create con instructions semplici.
        // Il modello dice la disclosure e prosegue naturalmente con la risposta + tool.
        send({
          type: 'response.create',
          response: {
            instructions: `Your reply must begin with this exact sentence, verbatim: "${disclosureText}" — then continue naturally in ${detectedLang.toUpperCase()} to help the caller, calling tools as needed.`,
          },
        });
      } else {
        send({ type: 'response.create' });
