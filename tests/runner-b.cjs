...
async function runConversation(systemPrompt, userTurns) {
  ...
  const aiReplies = [];
  const artifacts = [];
  let lastKnownEventId = null;   // v8.1: tracciato da crea_prenotazione e trova_prenotazione, iniettato in modifica_prenotazione se mancante
    ...
    await new Promise((resolve, reject) => {
      ...
      const handler = async (raw) => {
        ...
        if (msg.type === 'response.function_call_arguments.done') {
          ...
          // 🛡️ v8.1 Difesa: se modifica_prenotazione arriva senza eventId,
          //    inietta l'ultimo eventId noto in questa conversazione (da crea
          //    o trova precedente). Il modello a volte "dimentica" eventId anche
          //    con schema required — non ne facciamo un dramma, lo aggiungiamo noi.
          if (name === 'modifica_prenotazione' && (!args.eventId || args.eventId === '')) {
            if (lastKnownEventId) {
              console.log(`  🛡️ inject eventId=${lastKnownEventId.substring(0,12)}... in modifica_prenotazione`);
              args.eventId = lastKnownEventId;
            }
          }

          let result;
          // 🛡️ v8.1: retry 1 volta se la chiamata va in abort (timeout backend)
          //    Il backend App Script è occasionalmente lento — un retry singolo
          //    con piccolo delay risolve la maggior parte degli abort transitori.
          for (let attempt = 0; attempt < 2; attempt++) {
            try { result = await executeTool(name, args); }
            catch (e) { result = { error: e?.message || String(e) }; }
            const isAbort = result?.error && /abort/i.test(result.error);
            if (!isAbort) break;
            if (attempt === 0) {
              console.log(`  🔁 retry ${name} (previous attempt aborted)`);
          ...
          if (name === 'crea_prenotazione' && result?.success) {
            ...
            lastKnownEventId = result.eventId;
          }
          // Aggiorna anche da trova_prenotazione, che è il modo principale per
          // il modello di riottenere l'eventId dopo la creazione.
          if (name === 'trova_prenotazione' && result?.found && result?.reservation?.eventId) {
            lastKnownEventId = result.reservation.eventId;
          }
...
async function runTest(test, systemPromptTemplate) {
  ...
  // 🛡️ v8.2: cleanup opzionale via env var CLEANUP_ARTIFACTS.
  //   Default: false — le prenotazioni create dai test NON vengono cancellate
  //     dopo l'esecuzione. Consente ispezione manuale dello stato finale
  //     nei Google Sheets e Calendar.
  //   Per ripristinare il comportamento vecchio: CLEANUP_ARTIFACTS=1
  if (process.env.CLEANUP_ARTIFACTS === '1') {
...
async function main() {
  ...
  const results = [];
  const INTER_TEST_DELAY_MS = 5000; // Anti rate-limit OpenAI Realtime API (v7.4.48+: prompt grande, TPM saturi con 2s)
