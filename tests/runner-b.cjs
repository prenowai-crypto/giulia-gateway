...
async function runConversation(systemPrompt, userTurns) {
  ...
  // v8.6: replica logica gateway v7.5.1 in `_toolModifica` e `_foundResult`.
  //   Il gateway di produzione mantiene `this._lastFound` in memoria di sessione
  //   e lo usa per iniettare eventId + merge campi in modifica_prenotazione.
  //   Il runner-b bypassava questa logica → test mostravano fail che NON
  //   esistono in produzione. Ora il runner replica esattamente il gateway.
  let lastFound = null;
    ...
    await new Promise((resolve, reject) => {
      ...
      const handler = async (raw) => {
        ...
        if (msg.type === 'response.function_call_arguments.done') {
          ...
          // v8.6: replica esattamente `_toolModifica` del gateway v7.5.1
          //   Se il modello chiama modifica_prenotazione senza eventId (limite
          //   noto di gpt-realtime-2.1-mini), inietto eventId + merge dei campi
          //   mancanti dall'ultimo lastFound. Come fa il gateway in produzione.
          if (name === 'modifica_prenotazione' && lastFound) {
            const hasEventId = args.eventId != null && String(args.eventId).trim() !== '';
            if (!hasEventId) {
              args.eventId = lastFound.eventId;
              console.log(`  🛡️  v8.6 gateway-like: inject eventId=${lastFound.eventId?.substring(0,12)}... da lastFound`);
            }
            // Merge campi mancanti (come fa _toolModifica del gateway)
            const hasNome = args.nome    != null && String(args.nome).trim()    !== '';
            const hasData = args.data    != null && String(args.data).trim()    !== '';
            const hasOra  = args.ora     != null && String(args.ora).trim()     !== '';
            const hasPpl  = args.persone != null && parseInt(args.persone, 10) > 0;
            if (!hasNome) args.nome    = lastFound.name;
            if (!hasData) args.data    = lastFound.date;
            if (!hasOra)  args.ora     = lastFound.time;
            if (!hasPpl)  args.persone = lastFound.people;
          }
          ...
          // v8.6: cattura lastFound da trova_prenotazione (come `_foundResult` del gateway)
