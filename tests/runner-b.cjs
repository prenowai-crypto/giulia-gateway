  ...
  for (const userText of userTurns) {
    ...
    if (!firstUserTurnHandled) {
      ...
      if (detectedLang !== 'it') {
        ...
        console.log(`  ⚖️  Injecting ${detectedLang.toUpperCase()} disclosure via session.update (Soluzione B)`);

        // Soluzione B — Stesso identico pattern del gateway v7.4.36:
        // 1. session.update con instructions temporanee (system-level = strict rules)
        // 2. response.create blank → il modello dice la disclosure
        // 3. Aspetto response.done
        // 4. session.update per ripristinare il system prompt originale
        // 5. response.create blank → il modello risponde normalmente
        await new Promise((resolve, reject) => {
          const to = setTimeout(() => {
            ws.off('message', handler);
            reject(new Error(`disclosure timeout`));
          }, TURN_TIMEOUT_MS);
          let discText = '';
          const handler = (raw) => {
          ...
          ws.on('message', handler);
          // Step 1-2: temp session + response
          send({
            type: 'session.update',
            session: {
              type: 'realtime',
    ...
    // Step 5 (or normal path for italian): response.create per la vera risposta
    send({ type: 'response.create' });
