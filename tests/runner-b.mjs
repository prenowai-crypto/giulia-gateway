...
// v7.7.21: state per replicare _lastFound del gateway openai-realtime.js.
// Il runner bypassa il gateway e chiama direttamente i wrapper backend.
// Ma il gateway fa un lavoro importante: dopo trova_prenotazione salva la
// reservation in _lastFound, e prima di modifica/cancella inietta
// eventId = _lastFound.id nei params al wrapper. Senza questa replica,
// modifica_prenotazione riceve params.eventId=undefined e restituisce
// {success:false, reason:'missing_eventid'} — ma il runner conta PASS
// v7.7.21: state per replicare _lastFound del gateway openai-realtime.js.
// v7.7.32 FIX B09-009 (2026-09-15): può essere OBJECT (single-result) o ARRAY
// (multi-result). Se multi-result, NON iniettiamo eventId in modifica/cancella —
// lasciamo che il modello passi nome+data e il tool wrapper backend (v7.7.31
// con safety net) fa la disambiguation server-side.
//
// Prima (bug): reservations[0] → sempre mapped[0] → cancellava/modificava la
// prenotazione SBAGLIATA quando c'erano più risultati per stesso nome.
let lastFoundReservation = null;
...
async function executeTool(name, args, testId) {
  ...
  // v7.7.21: inject eventId da lastFoundReservation (come fa gateway._toolModifica).
  // Il modello non passa eventId negli args di modifica/cancella (non è nello
  // schema tool esposto al modello dal runner). Il wrapper backend però lo
  // richiede. Lo iniettiamo qui usando la reservation trovata dall'ultima
  // trova_prenotazione riuscita in questo stesso test.
  //
  // v7.7.32 FIX B09-009: SOLO se lastFoundReservation è single-result (object).
  // Se è ARRAY (multi-result), NON iniettiamo — il tool wrapper backend v7.7.31
  // fa disambiguation usando nome+data che il modello passa.
  if (name === 'modifica_prenotazione' || name === 'cancella_prenotazione') {
    if (lastFoundReservation && !Array.isArray(lastFoundReservation) && lastFoundReservation.id) {
      markedArgs.eventId = lastFoundReservation.id;
    }
    // Se lastFoundReservation è array (multi-result), NON iniettiamo eventId:
    // il modello deve aver passato `nome` + `data` (dopo aver disambiguato con
    // il cliente); il wrapper backend v7.7.31 risolve nome+data → eventId
    // corretto con safety net multi-result.
  }
  ...
  try {
    ...
    switch (name) {
      case 'controlla_disponibilita':
        result = await controllaDisponibilitaTool(TEST_TENANT, markedArgs, meta);
        break;
      case 'crea_prenotazione':
        result = await creaPrenotazioneTool(TEST_TENANT, markedArgs, meta);
        break;
      case 'trova_prenotazione':
        result = await trovaPrenotazioneTool(TEST_TENANT, markedArgs, meta);
        // v7.7.21: salva la reservation trovata per successive modifica/cancella.
        // v7.7.32 FIX B09-009: se multi-result, salva l'ARRAY completo (non solo
        // reservations[0]). L'inject in modifica/cancella distingue array vs object.
        if (result?.found && result.reservation) {
          // Single-result: comportamento invariato
          lastFoundReservation = result.reservation;
        } else if (result?.found && Array.isArray(result.reservations) && result.reservations.length > 1) {
          // MULTI-RESULT: salva TUTTI (come array) invece di mapped[0]
          lastFoundReservation = result.reservations;
        } else if (result?.found && Array.isArray(result.reservations) && result.reservations.length === 1) {
          // Array con 1 solo elemento: tratta come single-result
          lastFoundReservation = result.reservations[0];
        }
        break;
      case 'modifica_prenotazione':
        result = await modificaPrenotazioneTool(TEST_TENANT, markedArgs, meta);
        break;
      case 'cancella_prenotazione':
        result = await cancellaPrenotazioneTool(TEST_TENANT, markedArgs, meta);
        break;
      case 'richiedi_evento':
        result = await richiediEventoTool(TEST_TENANT, markedArgs, meta);
        break;
      case 'info_locale':
        result = await infoLocaleTool(TEST_TENANT, markedArgs, meta);
        break;
      case 'trasferisci_al_ristorante':
        result = { trasferita: true, istruzione: 'transfer stubbed for tests' };
        break;
      default:
        result = { errore: `unknown tool: ${name}` };
    }
    return result;
  } catch (err) {
...
// v7.7.24: helper per confronto value tollerante ai formati (time, date).
// Il modello può passare "22:00" o "22:00:00" per l'ora, o "2026-08-22" o
// "2026-08-22T00:00:00.000Z" per la data. Confronto normalizzato.
function argValueMatches(expected, actual) {
  if (expected === actual) return true;
  if (expected == null || actual == null) return expected === actual;
  const e = String(expected).trim();
  const a = String(actual).trim();
  if (e === a) return true;
  // Normalizza time: strip seconds
  const timeE = e.match(/^(\d{1,2}:\d{2})(:\d{2})?$/);
  const timeA = a.match(/^(\d{1,2}:\d{2})(:\d{2})?$/);
  if (timeE && timeA) return timeE[1] === timeA[1];
  // Normalizza date: strip time part se presente
  const dateE = e.match(/^(\d{4}-\d{2}-\d{2})/);
  const dateA = a.match(/^(\d{4}-\d{2}-\d{2})/);
  if (dateE && dateA && !timeE && !timeA) return dateE[1] === dateA[1];
  return false;
}
...
function assertToolCalls(actual, spec) {
  ...
  for (const expected of spec.expectedToolCalls || []) {
    const found = actual.toolCalls.find(tc =>
      ...
      (!expected.argsContain || Object.entries(expected.argsContain).every(([k, v]) => argValueMatches(v, tc.args[k])))
    );
...
function summarizeToolResult(name, result) {
  ...
  if (name === 'crea_prenotazione') {
    // v7.7.23: wrapper Postgres restituisce {success, eventId} (non {creata})
    if (result.success && result.eventId) return `✅ creata (eventId: ${String(result.eventId).substring(0, 20)}...)`;
    if (result.eventId) return `creata (eventId: ${String(result.eventId).substring(0, 20)}...)`;
    return result.creata ? `✅ creata` : `❌ ${result.reason || result.motivo || 'failed'}`;
  }
  // v7.7.23: fix campi restituiti dai wrapper Postgres.
  // Il runner bypassa gateway._toolXxx (che faceva mapping success→trovata/aggiornata/ecc.)
  // e chiama direttamente i wrapper che usano il campo `success` (o `found` per trova).
  if (name === 'trova_prenotazione') return (result.found || result.trovata) ? `✅ trovata` : '❌ non trovata';
  if (name === 'modifica_prenotazione') return (result.success || result.aggiornata) ? '✅ ok' : `❌ ${result.reason || result.motivo || 'failed'}`;
  if (name === 'cancella_prenotazione') return (result.success || result.cancellata) ? '✅ cancellata' : `❌ ${result.reason || result.motivo || 'failed'}`;
  if (name === 'richiedi_evento') return (result.success || result.registrata) ? '✅ evento registrato' : `❌ ${result.reason || result.motivo || 'failed'}`;
...
async function main() {
  ...
  for (let i = 0; i < allTests.length; i++) {
    const t = allTests[i];
    // v7.7.21: reset lastFoundReservation per ogni nuovo test (isolamento)
    lastFoundReservation = null;
