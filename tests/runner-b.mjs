...
// ═══════════════════════════════════════════════════════════════════════════════
// MODEL SWITCH — 2026-09-10 (allineato al gateway v8.2 in produzione)
// ═══════════════════════════════════════════════════════════════════════════════
// Default: 'gpt-realtime-2.1' (full) per rispecchiare il comportamento del gateway
// deployato su Render. Override rapido via env var senza modificare il codice:
//   REALTIME_MODEL=gpt-realtime-2.1-mini node runner-b.mjs --batch B02
// ═══════════════════════════════════════════════════════════════════════════════
...
// perché il tool call è stato fatto. Risultato: false positive, DB mai
// aggiornato. Fix: replichiamo lastFoundReservation + inject qui.
...
async function executeTool(name, args, testId) {
  ...
  if (name === 'modifica_prenotazione' || name === 'cancella_prenotazione') {
    if (lastFoundReservation && lastFoundReservation.id) {
  ...
  try {
    ...
    switch (name) {
      ...
      case 'trova_prenotazione':
        ...
        // Il wrapper trova-prenotazione restituisce {found, reservation:{id,...}}.
        // Prendiamo la prima reservation trovata (o l'unica se count===1).
        if (result?.found && result.reservation) {
          lastFoundReservation = result.reservation;
        } else if (result?.found && Array.isArray(result.reservations) && result.reservations.length > 0) {
      ...
      if (msg.type === 'session.created') {
        send({
          ...
          session: {
            ...
            // ═════════════════════════════════════════════════════════════════
            // REASONING — 2026-09-10 (allineato al gateway v8.2 in produzione)
            // ═════════════════════════════════════════════════════════════════
            // Con full model 'gpt-realtime-2.1' attivo reasoning al livello
            // MINIMO ('low') per contenere costi mantenendo il boost di
            // capacità del full. Se REALTIME_MODEL è il mini, questo parametro
            // dovrebbe essere ignorato dal backend OpenAI (test-safe).
            // ═════════════════════════════════════════════════════════════════
