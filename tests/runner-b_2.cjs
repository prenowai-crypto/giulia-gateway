...
const { callAppsScript, safeCancelByEventId } = require('./utils.cjs');
...
async function runConversation(systemPrompt, userTurns) {
    ...
    await new Promise((resolve, reject) => {
      ...
      const handler = async (raw) => {
        ...
        if (msg.type === 'response.function_call_arguments.done') {
          ...
          if (name === 'crea_prenotazione' && result?.eventId) artifacts.push(result.eventId);
        ...
        if (msg.type === 'response.done') {
          if (currentResponseText.trim()) aiReplies.push(currentResponseText.trim());
  ...
  return { toolCalls, aiReplies, artifacts };
}
...
function assertLanguage(actual, spec) {
  ...
  const markers = {
    it: /\b(sono|prenotazione|grazie|salve|buongiorno|perfetto|un attimo|certo|va bene|sabato|domenica|lunedì|martedì|mercoledì|giovedì|venerdì|persone|tavolo|conferm[oa]|prenotato)\b/,
    ...
    fr: /\b(bonjour|réservation|merci|s'il vous plaît|parfait|un instant|bien sûr|samedi|dimanche|lundi|mardi|mercredi|jeudi|vendredi|personnes|table|réservé)\b/,
    de: /\b(hallo|reservierung|danke|bitte|einen moment|natürlich|perfekt|samstag|sonntag|montag|dienstag|mittwoch|donnerstag|freitag|personen|tisch|reserviert)\b/,
    es: /\b(hola|reserva|gracias|por favor|un momento|perfecto|claro|sábado|domingo|lunes|martes|miércoles|jueves|viernes|personas|mesa|reservado)\b/,
  };
  const rx = markers[spec.replyLanguage];
  if (!rx) return [];
  if (!rx.test(last)) return [`expected reply language '${spec.replyLanguage}' — got: ${last.substring(0, 200)}`];
  if (spec.replyLanguage !== 'it' && markers.it.test(last)) {
    return [`language mix: expected ${spec.replyLanguage} but Italian markers found in: ${last.substring(0, 200)}`];
  }
  return [];
}
...
async function runTest(test, systemPromptTemplate) {
  ...
  for (const eid of convo.artifacts) await safeCancelByEventId(eid);
  return {
    id: test.id,
...
async function main() {
  ...
  const files = fs.readdirSync(DATA_DIR).filter(f => f.startsWith('b-') && f.endsWith('.json')).sort();
  const allTests = [];
